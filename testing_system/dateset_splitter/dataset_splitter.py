...
class TabularDatasetSplitter:
    def __init__(self, path_to_csv_data,
                 path_to_produced_train,
                 path_to_produced_test,
                 ):
        self.path_to_csv_data = path_to_csv_data
        self.path_to_produced_train = path_to_produced_train
        self.path_to_produced_test = path_to_produced_test

    def split(self, test_size=0.3) -> None:
        csv_sep = detect_csv_delimiter(self.path_to_csv_data)

        df = pd.read_csv(self.path_to_csv_data, sep=csv_sep)
        df_train, df_test = train_test_split(df, test_size=test_size)

        df_train.to_csv(path_or_buf=self.path_to_produced_train, sep=csv_sep)
        df_test.to_csv(path_or_buf=self.path_to_produced_test, sep=csv_sep)
